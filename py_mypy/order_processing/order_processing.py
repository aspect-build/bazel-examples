...
def create_order(
    customer_id: int, items: List[tuple[int, int]], shipping_address: str | None = None
) -> Order:
    ...
    for product_id, quantity in items:
        ...
        product = Product(
            product_id=product_id, name=f"Product {product_id}", price=10.0 * product_id
        )
    ...
    )  # Order id would be generated in real application.
    save_order(order)
    return order

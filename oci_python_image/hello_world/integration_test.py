...
def OCIImageContainer(image):
    ...
    try:
        ...
        cache_res = client.images.load(probe.getvalue())
        logger.debug("Image was a cache hit")
        return DockerContainer(cache_res[0].id)
    ...
    for i in range(1, len(layers)):
        ...
        try:
            ...
            logger.debug(
                "Layer %s was a cache hit, image id: %s",
                layer["digest"],
                probe_res[0].id,
            )
        except docker.errors.ImageLoadError as e:
    ...
    logger.debug(
        "Loaded image id: %s (OCI config digest: %s)",
        load_res[0].id,
...
def test_wait_for_hello():
    ...
    with OCIImageContainer("oci_python_image/hello_world/image") as container:
        wait_for_logs(container, "why is this slow?")
